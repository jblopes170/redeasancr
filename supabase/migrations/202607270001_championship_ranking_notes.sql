-- Exibe as notas e os pontos de cada etapa no resultado do campeonato.
-- Os campos stage_*_score continuam representando pontos de ranking.
...
with stage_totals as (
  select
    ...
    sum(s.final_score) as stage_note
...
stage_ranked as (
  select
    ...
    row_number() over (
      ...
      order by stage_totals.stage_note desc, stage_totals.competitor_name asc
    ) as stage_position
  from stage_totals
),
scored as (
...
base as (
  select
    ...
    scored.horse_id,
    scored.horse_name,
    sum(case when scored.stage = 1 then scored.stage_note else 0 end) as stage_1_note,
    sum(case when scored.stage = 2 then scored.stage_note else 0 end) as stage_2_note,
    sum(case when scored.stage = 3 then scored.stage_note else 0 end) as stage_3_note,
